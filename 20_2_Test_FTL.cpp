...
std::vector<Request> test_1_3_reqs = { {W,0,8} , {D,0,8}, {W,0,8}};

//TODO: 문제1. (1-4) - FTL의 모듈 코드에 대해서 statement coverage 100%가 만족되는 요청 입력
std::vector<Request> test_1_4_reqs = 
// {  {R,7,1}, {D,7,1}, {M,0,5}, {W,0,8}, {W,0,6}, {D,0,8}, {W,0,4}, {M,0,2}, {W,4,4}, {M,0,4}, {D,0,4},	{W,0,6},{R,0,3} };


{ {W,0,8}, {W,0,1}, {D,0,4}, {D,0,2}, {M,6,2}, {W,0,4}, {M,0,2}, {R,0,2}, {R,4,2}, {W,2,1}, {W,7,1}, {M,4,5} };
 
 //{W,2,5},  {M, 0,3}, {W,0,1}, {M,4,4}, {M,4,1}, {R, 1,2}, {R,5,2} };
...
private:
	...
	int findFreePhysicalBlock(int count)
	{
		...
		for (int i = 0; i <= MAX_BLOCK_COUNT - count; i++) {
			...
			for (j = i; j < i + count; j++) {
				if (physicalStatus_[j] == 1) break;
				sum+=eraseStatus_[j];  // index 를 j가 아닌 i로 해서 오답나옴.
		...
		if ( found == 1 )
			return minFirstBlock;
		
		return -1;
		
		// for(int i=0; i<9; i++) {
		// 	if( arr[i] == arr[minFirstBlock] )
		// 		return i;
		// }
	}
...
int main()
{
	...
	printTestInputResult("Test_1_4 Input", test_1_4_reqs);
